...
def CryptDeriveKey(h, cipherAlgo, hashAlgo):
    ...
    pad1 = ''.join(chr(ord(h[i]) ^ 0x36) for i in range(hashAlgo.blockSize))
    pad2 = ''.join(chr(ord(h[i]) ^ 0x5c) for i in range(hashAlgo.blockSize))
    k = hashlib.new(hashAlgo.name, pad1).digest() + hashlib.new(hashAlgo.name, pad2).digest()
